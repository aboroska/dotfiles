...
" Author: Claudio Fleiner <claudio@fleiner.com> modified by Laszlo Nemeth 
" 		(replaced "german" by "american", "american" by "magyar"), erased -C ispell flag
" F6         - write file, spell file & highlight spelling mistakes
" <SHIFT>F6  - switch between german and hungarian spelling
...
:  let alter=system("echo ".expand("<cword>")." | /net/duna198/export/localhome/ethanl/programs/ispell/bin/ispell -a -d ".b:language." | sed -e '/^$/d' -e '/^[@*#]/d' -e 's/.*: //' -e 's/,//g' | gawk '{ for(i=1;i<=NF;i++) if(i<10) printf \"map %d :let r=SpellReplace(\\\"%s\\\")<CR> | echo \\\"%d: %s\\\" | \",i,$i,i,$i; }'")
:  if alter !=? ""
...
:  let b:mappings=system("/net/duna198/export/localhome/ethanl/programs/ispell/bin/ispell -l -d ".b:language." < ".expand("%")." | sort -u | sed 's/\\(.*\\)/syntax match SpellErrors \"\\\\<\\1\\\\>\" ".b:spell_options."| let b:spellerrors=b:spellerrors.\"\\\\\\\\\\\\\\\\|\\1\"/'")
:  exe b:mappings
:  let b:spellerrors=b:spellerrors."\\)\\>"
:  map <ESC>i :let @_=system("echo \\\*".expand("<cword>")." \| /net/duna198/export/localhome/ethanl/programs/ispell/bin/ispell -a -d ".b:language)<CR>:syn case match<cr>:exe "syn match SpellCorrected \"\\<".expand("<cword>")."\\>\" transparent contains=NONE ".b:spell_options<cr><cr><c-l>
:  map <ESC>u :let @_=system("echo \\\&".expand("<cword>")." \| /net/duna198/export/localhome/ethanl/programs/ispell/bin/ispell -a -d ".b:language)<CR>:syn case ignore<cr>:exe "syn match SpellCorrected \"\\<".expand("<cword>")."\\>\" transparent contains=NONE ".b:spell_options<cr><cr><c-l>
...
:  map <m-i> :let @_=system("echo \\\*".expand("<cword>")." \| /net/duna198/export/localhome/ethanl/programs/ispell/bin/ispell -a -d ".b:language)<CR>:syn case match<cr>:exe "syn match SpellCorrected \"\\<".expand("<cword>")."\\>\" transparent contains=NONE ".b:spell_options<cr><cr><c-l>
